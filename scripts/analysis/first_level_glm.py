        ...
        for moco in list(mocos.keys()):
            ...
            for space in spaces: 
                ...
                if mocos[moco]==False:
                    ...
                    filepath=os.path.join(DATA_DIR, 'rawdata',f'sub-{subj:02}',f'ses-{ses}','onav_data',onav_files[subj]) ,
                ...
                for contrast in contrasts_names:
                    ...
                    nib.save(z_map, os.path.join(FMRIPREP_PATH,f'sub-{subj:02}',f'ses-{ses}','stats',f'sub-{subj:02}_ses-{ses}_zmap_{contrast}_{space}_{moco}.nii'))
                    ###########FPR at 0.001###########
                    thresholded_map, threshold = threshold_stats_img(z_map,alpha=0.001,
                    height_control='fpr',two_sided=True)
                    plot_activations(z_map, anat_file, gm_file, threshold, contrast, moco, space,
                    ...
                    disp_surf_activations(space, z_map,FREESURFER_PATH,
                                          os.path.join(FMRIPREP_PATH,f'sub-{subj:02}',f'ses-{ses}'),
                                          contrast, moco, subj, ses )
                    i=i+1
